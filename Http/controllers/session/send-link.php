...
if(Session::has('token') and Session::has('token_expires') > time() and empty($form->errors())) {
    Session::flash('message', 
                        [
                            'success' => str_replace(':minutes', round(abs($_SESSION['token_expires'] - time()) / 60), Lang::get('userForms.reset_token_valid'))
                        ]);
...
if (!($user = App::resolve(Database::class)
    ...
    ->find()) and empty($form->errors()))
        $form->appendError('null_user', Lang::get('userForms.null_user'));
...
if($active_token and $user['used'] == 'No') {
    $form->appendError('reset_active',  str_replace(':minutes', round(abs(strtotime($user['expires_at']) - time()) / 60), Lang::get('userForms.reset_token_valid')));
}
...
if(empty($form->errors())) {
        ...
        Session::flash('message', ['success' => Lang::get('userForms.reset_success')]);
